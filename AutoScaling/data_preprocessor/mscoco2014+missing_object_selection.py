"given regions and objects, decide which object does not appear in the regions"
...
class InstructData:
    ...
    def create_missing_object_selection_inst(self, save_path, input_dict, num_inst, split):
    ...
    def create_data(self):
        ...
        if self.task_type == 'missing_object_selection':
            save_path = self.out_data_dir / 'valid.jsonl'
            input_dict = json.load(open(self.val_dir, 'r'))
            self.create_missing_object_selection_inst(save_path, input_dict, self.num_val, 'valid')
            
            save_path = self.out_data_dir / 'train.jsonl'
            input_dict = json.load(open(self.train_dir, 'r'))
            self.create_missing_object_selection_inst(save_path, input_dict, self.num_train, 'train')
        else:
