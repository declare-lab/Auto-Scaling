"""
given object, generate all the regions in the image that have this object
"""
...
class InstructData:
    ...
    def create_region_generation_inst(self, save_path, input_dict, num_inst, split):
        with open(save_path,'w') as fout:
            ...
            for k in random.sample(list(img2obj.keys()),len(img2obj)):
                ...
                select_region = []
                for temp in v:
                    if temp[1] == select_obj:
                        select_region.append(temp[0])
                assert len(select_region) > 0
    ...
    def create_data(self):
        ...
        if self.task_type == 'region_generation':
            save_path = self.out_data_dir / 'valid.jsonl'
            input_dict = json.load(open(self.val_dir, 'r'))
            self.create_region_generation_inst(save_path, input_dict, self.num_val, 'valid')
            
            save_path = self.out_data_dir / 'train.jsonl'
            input_dict = json.load(open(self.train_dir, 'r'))
            self.create_region_generation_inst(save_path, input_dict, self.num_train, 'train')
        else:
